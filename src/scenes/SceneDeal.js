...
export default class SceneDeal extends Phaser.Scene {
  ...
  handleAllStopped(mainMenu) {
    ...
    if (this.winData.totalWin > 0 || this.winData.freeSpinsAwarded > 0) {
      ...
      this.time.delayedCall(300, () => this.final(mainMenu));
    }
  }
  ...
  showWinSequence(mainMenu) {
    ...
    if (this.winData.freeSpinsAwarded > 0) {
      ...
      const delay = this.winData.totalWin > 0 ? 1500 : 500;
      this.time.delayedCall(delay, () => this.final(mainMenu));
    }
  }

  showWinPopup(totalWin) {
    const currentBet = this.winData.isFreeSpin ? this.winData.bet * this.winData.activeLines : this.winData.totalBet;
  ...
  drawAllWinLines() {
    ...
    this.time.delayedCall(1200, () => graphics.destroy());
  }
  ...
  final(mainMenu, fromFS = false) {
    ...
    if (this.winData.totalWin > 0) {
      mainMenu.animateBalance(this.winData.newBalance, this.winData.totalWin, this.winData.balanceBeforeSpin);
    }

    if (this.winData.freeSpinsRemaining > 0) {
      mainMenu.panel.setFreeSpinMode(true, this.winData.freeSpinsRemaining);
    } else if (this.winData.isFreeSpin && this.winData.freeSpinsRemaining === 0) {
      mainMenu.exitFreeSpins();
    }

    const keepLocked = mainMenu.isAutoSpin || this.winData.freeSpinsRemaining > 0;
    if (!keepLocked) {
      mainMenu.panel.setLocked(false);
    }

    if (keepLocked) {
      const delay = this.winData.totalWin > 0 ? 1000 : 300;
