...
export default class SceneMainMenu extends Phaser.Scene {
  ...
  create() {
    ...
    this.events.on(GameEvents.UI.UNLOCK_INTERFACE, () => this.panel.setLocked(false));

    this.events.on(GameEvents.UI.CHANGE_LINES, (step) => {
      this.stats.activeLines = Phaser.Math.Clamp(this.stats.activeLines + step, 1, 20);
      this.syncPanel();
    });
    ...
    this.events.on(GameEvents.UI.TOGGLE_AUTO, () => {
      ...
      if (this.panel.updateAutoSpinState) this.panel.updateAutoSpinState(this.isAutoSpin);
  ...
  animateBalance(targetBalance, winAmount, startBalance = null) {
    const startValue = startBalance !== null ? startBalance : targetBalance - winAmount;
